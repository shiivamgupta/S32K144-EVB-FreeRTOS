...
void rxCallback(void *driverState, uart_event_t event, void *userData)
{
    ...
    if (event == UART_EVENT_RX_FULL)
    {
        ...
        if ((buffer[bufferIdx] != '\n') && (bufferIdx != (BUFFER_SIZE - 2U)))
        {
            ...
            bufferIdx++;
            LPUART_DRV_SetRxBuffer(INST_LPUART0, &buffer[bufferIdx], 1U);
        }
    }
}
...
//		vTaskSuspend(NULL);
		...
		LPUART_DRV_SendData(INST_LPUART0, (uint8_t *)Message, strlen(Message));
		vTaskDelay(1000);
...
static void prvSetupHardware( void )
{
	...
	/* Install the callback for rx events */
	  ...
	  LPUART_DRV_InstallTxCallback(INST_LPUART0,txCallback,NULL);
